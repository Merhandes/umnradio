    ...
    <div
        ...
        <div class="w-full">
            <a href="/dashboard/users"
                class="bg-white border-2 border-black text-black rounded-lg px-3 py-2 font-poppins font-bold justify-center align-middle items-center hover:invert hover:cursor-pointer hover:bg-[#fde0c5] transition ease-in-out delay-100 duration-200">Back
                to Dashboard</a>
        </div>
        <div
            ...
            <h2 class="text-2xl font-bold font-poppins">EDIT USER</h2>
            <form action="/dashboard/users/{{ $user->id }}/update" method="POST" enctype="multipart/form-data"
                ...
                <div class="gap-4 flex flex-wrap">
                    ...
                    <div>
                        <p>Angkatan</p>
                        <input type="number" name="year" value="{{ old('year', $user->year) }}"
                            class="border-1 border-black py-2 px-3 text-lg font-poppins border rounded-lg mt-1 shadow w-full">
                        @error('year')
                            <div class="text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea name="address" id="" value="{{ old('address') }}"
                            class="border-1 border-black py-2 px-3 text-lg font-poppins border rounded-lg mt-1 shadow w-full">{{ $user->address }}</textarea>
                        @error('address')
                            <div class="text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    </div>
                    <div>
                        ...
                        <select name="role" id="role_select"
                            ...
                            <option disabled selected value> -- add role -- </option>
                            @foreach ($roledefs as $roledef)
                                @if ($roledef->id > 1 && !in_array($roledef->id, $userRoles))
                                    <option value='{{ $roledef->id }}'>{{ $roledef->id }} -
                                        {{ $roledef->name }}</option>
                                @endif
                            @endforeach
                        </select>
        ...
        <div
            class="bg-white drop-shadow-[0_5px_5px_rgba(0,0,0,0.25)] rounded-lg flex flex-col justify-center items-baseline align-top p-6 h-fit">
            <h2 class="text-lg font-bold font-poppins">Roles</h2>
            @foreach ($user->roles as $userRole)
                <div class="px-2 py-1 border border-1 border-black shadow drop-shadow-lg rounded-md">
                    {{ $userRole->role }} - {{ $userRole->roleDef->name }}
                    @if ($userRole->roleDef->name != 'admin' && $userRole->roleDef->name != 'superadmin')
                        <form action="/dashboard/users/role/{{ $userRole->id }}/delete" method="POST"
